...
{ 
	...
	// log file
	string log_filename = input_prefix + "_log.txt";
	...
	ofstream os_log(log_filename);

	// Ground truth vector
	transitions trans;

	cout << "Ground truth file (" << gt_filename << ") ";
	os_log << "Ground truth file (" << gt_filename << ") ";
	...
	os_log << "Comparison file (" << comparison_filename << ") ";
	...
	os_log << "Differences file (" << diffs_filename << ") ";
	ifstream is_diffs(diffs_filename);
	if (is_diffs) {
		cout << "already present: video won't be analyzed.\n";
		os_log << "already present: video won't be analyzed.\n";
		cout << "Reading differences... ";
		os_log << "Reading differences... ";
		...
		os_log << "not found: video will be analyzed.\n";
		cout << "Analyzing video... \n";
		os_log << "Analyzing video... \n";
	...
	for (double w=0.5; w<=W; w += 0.5) {
		...
		if (!trans.empty()) {
			...
			os_log << "W      :\t" << w << "\n";
			os_log << "Total  :\t" << ev._p._tot << "\t" << F1 << "\n";
			os_log << "Abrupt :\t" << ev._p._split[transition::abrupt] << "\n";
			os_log << "Gradual:\t" << ev._p._split[transition::gradual] << "\n";
			os_log << "\n";
	...
	if (!trans_comparison.empty()) {
		...
		os_log << "Comparison with " << comparison_filename << ":\n";
		os_log << "Total  :\t" << ev._p._tot << "\t" << F1 << "\n";
		os_log << "Abrupt :\t" << ev._p._split[transition::abrupt] << "\n";
		os_log << "Gradual:\t" << ev._p._split[transition::gradual] << "\n";
		os_log << "\n";
		
		string strrun = "start " + comparison_filename;
		system(strrun.c_str());
	}

	cout << "Writing transitions (" << trans_filename << ")... ";
	os_log << "Writing transitions (" << trans_filename << ")... ";
	ofstream os_trans(trans_filename);
	if (!os_trans) {
		cerr << "Unable to open transitions file for writing.\n";
		os_log << "Unable to open transitions file for writing.\n";
		exit(EXIT_FAILURE);
	}
	...
	if (true) {
		...
		os_log << "Writing shots (" << shots_filename << ")... ";
		...
		if (!os_shots) {
			...
			os_log << "Unable to open shots file for writing.\n";
			exit(EXIT_FAILURE);
		}
	...
	os_log.close();
	string strrunlog = "start " + log_filename;
	system(strrunlog.c_str());
}
